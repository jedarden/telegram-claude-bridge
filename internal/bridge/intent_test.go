...
func TestParseCommand(t *testing.T) {
	tests := []struct {
		...
		{`go test ./... -v`, []string{"go", "test", "./...", "-v"}, false},
	...
	for _, tc := range tests {
		t.Run(tc.input, func(t *testing.T) {
			parts, err := parseCommand(tc.input)
			if tc.err && err == nil {
			...
			if !tc.err {
				if len(parts) != len(tc.parts) {
					t.Errorf("parts: got %v, want %v", parts, tc.parts)
				}
				for i := range parts {
					if parts[i] != tc.parts[i] {
						t.Errorf("parts[%d]: got %q, want %q", i, parts[i], tc.parts[i])
					}
				}
			}
		})
...
func TestSplitParallelPrompts(t *testing.T) {
	tests := []struct {
		...
		{"single task", 1},
		{"\n---\n", 0}, // empty after trim
		{"task1\n---\n\n---\ntask3", 2}, // empty middle filtered
	}

	for _, tc := range tests {
		t.Run(tc.input, func(t *testing.T) {
			prompts := splitParallelPrompts(tc.input)
			if len(prompts) != tc.count {
				t.Errorf("got %d prompts, want %d: %v", len(prompts), tc.count, prompts)
			}
		})
	}
}
